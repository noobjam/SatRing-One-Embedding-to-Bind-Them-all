...
def parse_args():
    ...
    p.add_argument("--batch-size", type=int, default=32, help="Per-GPU batch size (default: 32)")
    p.add_argument("--epochs", type=int, default=100)
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--num-workers", type=int, default=16, help="Total data workers (default: 16)")
    p.add_argument("--save-dir", type=str, default="checkpoints")
    p.add_argument("--sequence-length", type=int, default=5)
    p.add_argument("--accumulation-steps", type=int, default=1, help="Gradient accumulation steps")
    p.add_argument("--use-amp", action="store_true", default=True, help="Use BF16 mixed precision")
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if rank == 0 and batch_idx % 10 == 0:
                ...
                print(f"Epoch {epoch:3d} | Step {batch_idx:4d}/{len(loader)} | "
                      f"Loss: {loss.item()*args.accumulation_steps:.4f} | "
                      f"All: {l_all.item():.3f} Season: {l_season.item():.3f} "
                      f"Sensor: {l_sensor.item():.3f} Semantic: {l_semantic.item():.3f} | "
                      f"LR: {lr:.2e}")
