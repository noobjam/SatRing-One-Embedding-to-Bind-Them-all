...
def main():
    ...
    if rank == 0:
        print(f"Starting DDP training: world_size={world_size}, local_rank={local_rank}, device={device}")
        print("Args:", args)
    ...
    backbone = TRACE(dim=1024, num_heads=8, temporal_depth=2, H=14, W=14, aggregate='mean').to(device)
    projector = ProjectionHead(in_dim=1024, hidden_dim=512, out_dim=args.proj_dim).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (anchor_seq, pair_seq, label) in enumerate(loader):
            anchor_seq = anchor_seq.to(device, non_blocking=True)
            pair_seq = pair_seq.to(device, non_blocking=True)
            ...
            if rank == 0 and epoch == 0 and batch_idx == 0:
                ...
                print(f"anchor_seq: {anchor_seq.shape}, pair_seq: {pair_seq.shape}, label: {label.shape}")
            ...
            with torch.cuda.amp.autocast(enabled=True, dtype=autocast_dtype):
                ...
                if rank == 0 and epoch == 0 and batch_idx == 0:
                    print(f"anchor_pixel: {anchor_pixel.shape}, pair_pixel: {pair_pixel.shape}")
                    print(f"anchor_feat: {anchor_feat.shape}, pair_feat: {pair_feat.shape}")
                ...
                if rank == 0 and epoch == 0 and batch_idx == 0:
                    print(f"z_a: {z_a.shape}, z_b: {z_b.shape}, mask_pos: {mask_pos.shape}, positive_count: {mask_pos.sum().item()}")
                    print("=== End debug shapes ===")

                if mask_pos.sum() == 0 or (~mask_pos).sum() == 0:
                    continue

                loss = info_nce_loss(z_a[mask_pos], z_b[mask_pos], temperature=args.temperature) / args.accum_steps
            ...
            if global_step % 50 == 0 and rank == 0:
                ...
                print(f"[Rank {rank}] Epoch {epoch+1} Step {global_step} Batch {batch_idx+1}/{len(loader)} "
                      f"Loss {loss.item() * args.accum_steps:.4f} Avg {avg_loss_now:.4f} Time {elapsed:.1f}s")

        if iters == 0:
            reduced_avg_loss = float('inf')
        else:
            local_avg = torch.tensor(epoch_loss / max(1, iters), device=device, dtype=torch.float32)
            dist.all_reduce(local_avg, op=dist.ReduceOp.SUM)
            reduced_avg_loss = local_avg.item() / world_size

        if rank == 0:
            print(f"Epoch {epoch+1} finished. Avg loss (all ranks): {reduced_avg_loss:.4f}")
            ckpt = {
                "epoch": epoch + 1,
